...
public record WearSettings(boolean enabled, Range lines, Map<String, Range> overrides,
    ...
    public static WearSettings readConfig() {
        ...
        for (String entry : ClientConfig.BREAK_LINE_OVERRIDES.get()) {
            ...
            overrides.put(net.minecraft.resources.ResourceLocation.parse(parts[0].trim()).toString(),
    ...
    public record Range(int min, int max) {
        public Range {
            int lower = Math.clamp(Math.min(min, max), 0, 8);
            max = Math.clamp(Math.max(min, max), 0, 8);
            min = lower;
        }
    }
}
