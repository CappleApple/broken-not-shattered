...
class WearPatternTest {
    @Test
    void separatelyBrokenItemsHaveIndependentSeedsAndCopiesKeepTheSavedPattern() {
        ItemStack first = new ItemStack(Items.GOLDEN_PICKAXE);
        first.setDamageValue(first.getMaxDamage());
        ItemStack second = first.copy();
        BreakPatternData.ensureSeed(first);
        BreakPatternData.ensureSeed(second);
        var before = first.getComponents();
        WearPattern a = BrokenAppearance.pattern(first);
        WearPattern b = BrokenAppearance.pattern(second);
        assertNotNull(a);
        assertNotNull(b);
        assertNotEquals(a.seed(), b.seed());
        assertNotEquals(a.cracks(), b.cracks());
        assertSame(a, BrokenAppearance.pattern(first));
        assertSame(a, BrokenAppearance.pattern(first.copy()));
        assertEquals(before, first.getComponents());
        first.setDamageValue(first.getMaxDamage() - 1);
        assertNull(BrokenAppearance.pattern(first), "repair must remove the effect on the very next render");
        first.setDamageValue(first.getMaxDamage());
        assertSame(a, BrokenAppearance.pattern(first));
    }

    @Test
    void renderingWaitsForTheSavedSeedAndDoesNotMutateAnUnseededStack() {
        ItemStack stack = new ItemStack(Items.GOLDEN_PICKAXE);
        stack.setDamageValue(stack.getMaxDamage());
        var before = stack.getComponents();
        assertNull(BrokenAppearance.pattern(stack));
        assertEquals(before, stack.getComponents());
        assertTrue(BreakPatternData.ensureSeed(stack));
        Long seed = BreakPatternData.seed(stack);
        assertFalse(BreakPatternData.ensureSeed(stack));
        assertEquals(seed, BreakPatternData.seed(stack));
    }
    ...
    void wearPreservesAlphaDoesNotEditInputAndRepeatsAcrossAnimationFrames() {
        ...
        try (NativeImage source = new NativeImage(16, 32, true)) {
            for (int y = 0; y < 32; y++) for (int x = 0; x < 16; x++) {
                source.setPixelRGBA(x, y, x == 0 ? 0 : 0x80AAEEFF);
            }
            try (NativeImage result = WornTextures.modify(source, pattern, 16, 16)) {
                int changed = 0;
                for (int y = 0; y < 16; y++) for (int x = 0; x < 16; x++) {
                    int original = source.getPixelRGBA(x, y);
                    int output = result.getPixelRGBA(x, y);
                    assertEquals(x == 0 ? 0 : 0x80AAEEFF, original);
                    assertEquals(original >>> 24, output >>> 24);
                    assertEquals(output, result.getPixelRGBA(x, y + 16));
