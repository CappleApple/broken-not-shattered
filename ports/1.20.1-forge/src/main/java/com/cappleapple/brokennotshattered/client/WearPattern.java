...
public record WearPattern(long seed, List<Crack> cracks, WearSettings settings) {
            ...
            for (int fork = 0; fork < forks; fork++) {
                ...
                if (random.nextDouble() < 0.4) {
                    ...
                    branches.add(branch(random, branch.points().get(split),
                        branch.points().get(branch.points().size() - 1).x() >= branch.points().get(0).x() ? -1 : 1,
    ...
    private static Path branch(SplittableRandom random, Point start, int direction, float length) {
        ...
        for (int step = 0; step < steps; step++) {
            ...
            Point next = new Point(
                com.cappleapple.brokennotshattered.core.Numbers.clamp(previous.x() + (float) Math.cos(angle) * stride, 0.035F, 0.965F),
                com.cappleapple.brokennotshattered.core.Numbers.clamp(previous.y() + (float) Math.sin(angle) * stride, 0.035F, 0.965F)
            );
            points.add(next);
            previous = next;
        }
    ...
    public record Path(List<Point> points) {
        ...
        public boolean through() {
            return points.get(0).y() == 0 && points.get(points.size() - 1).y() == 1;
        }
        ...
        double distanceSquared(double x, double y, double closest) {
            for (int i = 1; i < points.size(); i++) {
                ...
                double t = lengthSquared == 0 ? 0
                    : com.cappleapple.brokennotshattered.core.Numbers.clamp(((x - a.x()) * dx + (y - a.y()) * dy) / lengthSquared, 0, 1);
    ...
    private static int channel(double value) {
        return com.cappleapple.brokennotshattered.core.Numbers.clamp((int) Math.round(value), 0, 255);
    }
}
