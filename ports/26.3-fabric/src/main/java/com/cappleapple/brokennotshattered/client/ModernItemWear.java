...
public final class ModernItemWear {
    ...
    public static void apply(ItemStackRenderState state, ItemStack stack, ItemDisplayContext context) {
        ...
        for (int i = 0; i < access.bns$count(); i++) {
            List<BakedQuad> quads = ((com.cappleapple.brokennotshattered.mixin.ItemLayerAccessor)access.bns$layers()[i]).bns$quads().all();
            ...
            for (BakedQuad q : geometry) {
                ...
                var material = new BakedQuad.MaterialInfo(m.sprite(), m.layer(),
                    WearRenderType.wrap(m.itemRenderType(), texture), WearRenderType.wrap(m.itemGlintRenderType(), texture),
                    WearRenderType.wrap(m.itemGlintSpecialRenderType(), texture), m.tintIndex(), m.shadeDirectionOverride(), m.lightEmission());
                output.add(new BakedQuad(q.position0(), q.position1(), q.position2(), q.position3(),
                    q.packedUV0(), q.packedUV1(), q.packedUV2(), q.packedUV3(), q.direction(), material));
            }
            access.bns$layers()[i].setQuads(net.minecraft.client.resources.model.geometry.ItemQuads.split(output));
        }
    ...
    static BakedQuad quad(int[] vertices, Direction direction, BakedQuad source) {
        ...
        return new BakedQuad(pos[0], pos[1], pos[2], pos[3], uv[0], uv[1], uv[2], uv[3], direction, source.materialInfo());
    }
