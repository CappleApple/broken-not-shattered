        ...
        for (int i = 0; i < access.bns$count(); i++) {
            ...
            for (BakedQuad q : geometry) {
                ...
                var material = new BakedQuad.MaterialInfo(m.sprite(), m.layer(),
                    WearRenderType.wrap(m.itemRenderType(), texture), WearRenderType.wrap(m.itemGlintRenderType(), texture), WearRenderType.wrap(m.itemGlintSpecialRenderType(), texture), m.tintIndex(), m.shadeDirectionOverride(), m.lightEmission(), m.ambientOcclusion());
