...
public final class WornTextures {
    ...
    private static final Map<net.minecraft.client.resources.palette.PalettedTextureManager.Handle, PalettedSource> PALETTES = new WeakHashMap<>();
    private record PalettedSource(Identifier texture, net.minecraft.client.resources.palette.PaletteMapping mapping) {}
    public static void capturePalette(net.minecraft.client.resources.palette.PalettedTextureManager.Handle handle, Identifier texture, net.minecraft.client.resources.palette.PaletteMapping mapping) {
        PALETTES.put(handle, new PalettedSource(texture.withPath(path -> "textures/" + path + ".png"), mapping));
    }
    public static Identifier palette(net.minecraft.client.resources.palette.PalettedTextureManager.Handle handle, WearPattern pattern) {
        PalettedSource source = PALETTES.get(handle);
        return get(source == null ? handle.textureLocation() : source, pattern);
    }
    ...
    private static Identifier get(Object source, WearPattern pattern) {
        ...
        try {
            ...
            if(sprite!=null) {
                ...
                var resource=Minecraft.getInstance().getResourceManager().getResource(source instanceof PalettedSource palette ? palette.texture() : (Identifier)source);
                ...
                try(var stream=resource.get().open()) {input=NativeImage.read(stream); owns=true;
                    if(source instanceof PalettedSource palette) { NativeImage colored=input.mappedCopy(palette.mapping()); input.close(); input=colored; }}
            }
