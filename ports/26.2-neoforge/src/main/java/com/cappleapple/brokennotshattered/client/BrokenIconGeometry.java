...
final class BrokenIconGeometry {
    ...
    static List<BakedQuad> splitAll(List<BakedQuad> quads, boolean addInteriorCaps, WearPattern pattern) {
        ...
        for (BakedQuad quad : quads) {
            int[] originalVertices = ModernItemWear.vertices(quad);
            ...
            for (int[] vertices : splitVertices) {
                split.add(ModernItemWear.quad(vertices, quad.direction(), quad));
            }
        }
        if (addInteriorCaps) {
    ...
    private static void addInteriorCaps(List<BakedQuad> result, List<BakedQuad> originalQuads, WearPattern pattern) {
        ...
        for (BakedQuad front : originalQuads) {
            ...
            LayerKey layer = new LayerKey(front.materialInfo().sprite(), front.materialInfo().tintIndex());
            ...
            TextureAtlasSprite sprite = front.materialInfo().sprite();
            Vertex style = Vertex.read(ModernItemWear.vertices(front), 0);
            for (CapData cap : createCapVertexData(
                ...
                result.add(ModernItemWear.quad(cap.vertices(), cap.direction(), front));
            }
        }
    }

    private static boolean isGeneratedFrontFace(BakedQuad quad) {
        if (quad.direction() != Direction.SOUTH || ModernItemWear.vertices(quad).length != STRIDE * 4) {
            return false;
        }

        Bounds bounds = bounds(ModernItemWear.vertices(quad));
    ...
    private static float[] findDepth(List<BakedQuad> quads, LayerKey layer) {
        ...
        for (BakedQuad quad : quads) {
            if (quad.materialInfo().sprite() != layer.sprite() || quad.materialInfo().tintIndex() != layer.tintIndex()) {
                continue;
            }
            Bounds bounds = bounds(ModernItemWear.vertices(quad));
