...
public class BrokenItemGameTests implements FabricGameTest {
    ...
    public void durabilityPreservationAndRepair(GameTestHelper helper) {
        for(Item item:List.of(Items.WOODEN_PICKAXE,Items.DIAMOND_PICKAXE,Items.NETHERITE_PICKAXE,
            ...
            stack.hurtAndBreak(1,helper.getLevel(),null,ignored->calls.incrementAndGet());
            ...
            stack.hurtAndBreak(50,helper.getLevel(),null,ignored->calls.incrementAndGet());
    ...
    public void persistentSeedRoundTrip(GameTestHelper helper) {
        ...
        stack.hurtAndBreak(1,helper.getLevel(),null,ignored->{});Long seed=BreakPatternData.seed(stack);
        ...
        try{ItemStack.STREAM_CODEC.encode(buffer,restored);helper.assertTrue(seed.equals(BreakPatternData.seed(ItemStack.STREAM_CODEC.decode(buffer))),"Network lost seed");}finally{buffer.release();}
        restored.setDamageValue(0);restored.hurtAndBreak(restored.getMaxDamage(),helper.getLevel(),null,ignored->{});
    ...
    public void toolAndCombatSuppression(GameTestHelper helper) {
        ...
        var attributes=new HashSet<>();sword.forEachModifier(EquipmentSlot.MAINHAND,(attribute,modifier)->attributes.add(attribute));
        ...
        attributes.clear();armor.forEachModifier(EquipmentSlot.CHEST,(attribute,modifier)->attributes.add(attribute));
    ...
    public void tagsAndBackfill(GameTestHelper helper) {
        ...
        ignored.hurtAndBreak(1,helper.getLevel(),null,item->{});helper.assertTrue(ignored.isEmpty(),"Ignore tag did not permit destruction");
        var shatter=new ItemStack(Items.GOLDEN_AXE);shatter.setDamageValue(shatter.getMaxDamage()-1);
        shatter.hurtAndBreak(1,helper.getLevel(),null,item->{});helper.assertTrue(shatter.isEmpty(),"Shatters tag did not permit destruction");
    ...
    public void useAndEnchantments(GameTestHelper helper) {
        ...
        float damage=net.minecraft.world.item.enchantment.EnchantmentHelper.modifyDamage(helper.getLevel(),sword,player,helper.getLevel().damageSources().generic(),1);
        helper.assertTrue(damage==1,"Broken sword retained sharpness");helper.assertTrue(sword.getEnchantments().getLevel(sharpness)==5,"Enchantment storage changed");
        ...
        helper.assertTrue(bow.getUseDuration(player)==0,"Broken bow can charge");
