...
- Minecraft 1.21.1, Fabric Loader 0.19.5 or newer, and Java 21.
...
| Path | Behavior |
| --- | --- |
| `data/broken_not_shattered/tags/item/ignore.json` | Excludes items from this mod. |
| `data/broken_not_shattered/tags/item/shatters.json` | Allows normal destruction on exhaustion. |
| `data/broken_not_shattered/tags/item/protected.json` | Includes items with positive maximum durability even when vanilla does not consider them damageable. |
...
Run Gradle with JDK 21. The build selects a Java 21 toolchain and produces Java 21 bytecode.
...
`build` runs deterministic wear regression tests. `runGameTest` runs a dedicated Fabric test server. `runClientSmoke` renders test icons in a hidden, muted client, releases the mouse, saves `build/client-smoke/fabric-1.21.1-items.png`, and exits. Test mods and fixtures are in `src/gametest` and are excluded from the distribution JAR.
