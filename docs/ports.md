# Minecraft 26.2 / NeoForge port

This branch contains Broken Not Shattered 1.2.0 for Minecraft 26.2 / NeoForge under [ports/26.2-neoforge](../ports/26.2-neoforge/README.md). The repository root retains the original NeoForge 1.21.1 source and version 1.1.3. The original branch is [main](https://github.com/CappleApple/broken-not-shattered/tree/main).

## Installation

Install the matching port JAR on both client and server with Java 25. See the [target README](../ports/26.2-neoforge/README.md) for loader and dependency versions. Do not install multiple variants together. The `-sources.jar` and Forge `-slim.jar` are development outputs.
...
```powershell
cd ports/26.2-neoforge
.\gradlew.bat build runSmokeServer --max-workers=2
```

The installable JAR is written to `ports/26.2-neoforge/build/libs/`. From the repository root, the [Windows helper](../scripts/build-ports.ps1) collects the original and this port into `build/ports/`, with SHA-256 checksums:

```powershell
.\scripts\build-ports.ps1
.\scripts\build-ports.ps1 -Target 26.2-neoforge -Validate
```
