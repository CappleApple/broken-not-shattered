...
Validation performed September 19, 2026. The repository-root NeoForge 1.21.1 implementation remains version 1.1.3; this Fabric 1.21.1 port is version 1.2.0.
...
| Fabric 1.21.1 | Passed, 13 tests | 5 GameTests passed | Hidden, muted client rendered item icons; screenshot inspected |
...
The hidden, muted client rendered healthy and broken inventory icons with distinct seeds and enchantment glint. The screenshot was inspected at `build/client-smoke/fabric-1.21.1-items.png`, relative to the port directory. Equipped armor, trims, elytra, and third-party rendering combinations remain visually unverified.
...
See the [port guide](ports.md) and [target README](../ports/1.21.1-fabric/README.md) for JDK requirements, dependencies, and exact commands. The [Windows build helper](../scripts/build-ports.ps1) collects the original and this target's installable JARs and SHA-256 hashes in `build/ports/`; `-Validate` also runs the server gates.

JUnit reports are under `ports/1.21.1-fabric/build/test-results/test/`. Generated reports, logs, game directories, and screenshots are excluded from source control.
